...
export default class FoodOrder extends React.Component<{ foodOrders: { menu: foodMenu, quantity: number }[], enable: Function, delete: Function }, {loading: boolean}> {
  constructor(props: { foodOrders: { menu: foodMenu, quantity: number }[], enable: Function, delete: Function }) {
    super(props);
  ...
  async handleOrder() {
    ...
    for (let i = 0; i < this.props.foodOrders.length; i++) {
      await orderFet.chooseFood(1, this.props.foodOrders[i].menu.food_id, this.props.foodOrders[i].quantity);
    }
