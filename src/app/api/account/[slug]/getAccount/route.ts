...
export async function GET(req: Request, { params }: {params: {slug: number}}) {
    try {
        const data = await new database(new select<users>('users_id', 'username', 'role', 'email', 'password', 'created_at', 'token', 'picture', 'first_name', 'last_name', 'phone', 'booked').table('users').where('users_id', compare.EQUAL, params.slug)).query();
