...
export async function PATCH(req: Request) {
    ...
    if (data.username) {change.change('username', data.username)}
    if (data.password) {
        let hashPass: string = bcrypt.hash(data.password, 10);
        change.change('password', hashPass);
    }
    if (data.first_name) {change.change('first_name', data.first_name)}
    if (data.last_name) {change.change('last_name', data.last_name)}
    if (data.phone) {change.change('phone', data.phone)}
